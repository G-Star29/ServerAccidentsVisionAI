...
def mod_parsed_data():
    ...
    df = pd.read_csv('data/csv/parsed_output.csv')
    df['accident_occurred'] = 1
    df['ligth'] = df['ligth'].str.lower().str.strip()

    light_conditions = ["в темное время суток", "светлое время суток", "сумерки", "освещение включено",
    ...
    road_columns = ["Пыльное", "Обработанное противогололедными материалами", "Заснеженное", "Гололедица", "Мокрое",
                    "Сухое"]
    for column in road_columns:
        df[column] = df['road_conditions'].apply(lambda x: 1 if column in x else 0)

    nearby_columns = []
    with open('data/txt/nearby', 'r', encoding='utf-8') as file:
        for item in file:
    ...
    df_provided = pd.read_csv('data/csv/concatenated_data_final_v2.csv')
    ...
    df = df[
        df_provided.columns]  # Перестроение DataFrame в соответствии с порядком столбцов из предоставленной таблицы

    df.to_csv('data/csv/output_modified_test_final.csv', index=False)
    print("[PARSE] Сохраненные данные успешно модифицированы")